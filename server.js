    ...
    db.query(checkEmailSql, [email], (err, result) => {
        ...
        if (result.length > 0) {
          ...
          return
            ...
            const sql = 'INSERT INTO users (username, password, email, verification_token, email_verified, cookie, role, teachingTopic) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
            db.query(sql, [username, password, email, vtoken, true, cookie, role, teachingTopic], (err, result) => {
                if (err) {
                    console.error(err);
                    res.status(500).json({ error: 'Internal Server Error' });
                    return;
                ...
                res.status(200).json({ message: 'Successfully made custom account'})
...
app.post('/login', (req, res) => {
  ...
  db.query(sql, [usernameOrEmail, usernameOrEmail, password], (err, result) => {
      ...
      if (result.length > 0) {
          const user = result[0];
          res.cookie('EDUHOME-COOKIE', user.cookie);
          const role= result[0].role
...
app.put('/update-question/:id', (req, res) => {
  const questionId = req.params.id;
  const { question, answerA, answerB, answerC, answerD, trueAnswer, unit } = req.body;

  const query = 'UPDATE questions SET question=?, answerA=?, answerB=?, answerC=?, answerD=?, trueAnswer=?, unit=? WHERE id=?';
  const values = [question, answerA, answerB, answerC, answerD, trueAnswer, unit, questionId];
